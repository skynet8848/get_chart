...
name_list_minipdt=['zhaoguo-2','weiguo-2','chuguo-2','hanguo-2','gw-minipdt-quincy1','pdt-xian','pdt-tongchuan','slt28-palm','pdt-xian-2','pdt-tongchuan-2','nonibert','qinguo-2','shanhai','qinguo']
...
cnrd_owner_dict={'deanchen': ['calla', 'slt103-cicada', 'pdt-venus', 'pdt-mercury', 'pdt-mars', 'slt103-cricket', 'pdt-earth', 'choko', 'slt103-j7', 'guo', 'wan', 'pdt-jupiter', 'pdt-saturn'], 
				 ...
				 'darrenli': ['slt60-loquat', 'slt17-balsam', 'slt17-begonia', 'slt15-ginseng', 'slt60-j7', 'slt15-medlar', 'slt17-j7', 'slt60-lemon', 'pdt-thor', 'pdt-hulk', 'f13-33', 'f13-34', 'f14-23', 'pdt-forge-natt', 'f15-34', 'f15-36'], 
...
dut_list=['movers', 'pdt-thor', 'f15-34', 'slt60-lemon', 'slt60-cactus', 'slt123-wyk', 'g07-29', 'slt106-kb', 'poetry', 'pdt-honey', 'pdt-gw11', 'attacking', 'slt104-orange', 'pdt-pitaya', 'forge-pdt-10', 'pdt-gw5', 'pdt-melon', 'pdt-berry', 'slt102-python', 'slt13-carrot', 'slt15-medlar', 'slt17-balsam', 'slt12-jasmine', 'slt28-palm', 'slt11-orchid', 'greatwall-a', 'forge-lsys1', 'nonibert', 'qinguo-2', 'gw-minipdt-quincy1', 'pdt-xian-2', 'pdt-tongchuan-2', 'fig', 'pdt-xian', 'pdt-tongchuan', 'shanhai', 'qinguo', 'slt113-cavaliers', 'slt111-morphia', 'slt103-cicada', 'slt105-nongfu', 'slt112-warriors', 'sunshine', 'guo', 'pdt-winter', 'ibmdow-reg1', 'pdt-lotus', 'pdt-pepper', 'pdt-jupiter', 'slt7-tulip']
    ...
    for item in dut_list:
        ...
        if len(zero_ha_list)!=0:
            if max(zero_ha_list)!=len(error_ha_list):
                if max(zero_ha_list)>=192:
                    if item not in error_testbed_list:
                        error_testbed_list.append(item)
        if len(zero_fpc_list)!=0:
            if max(zero_fpc_list)>=192:
                if item not in error_testbed_list:
                    error_testbed_list.append(item)
                    
        if len(zero_session_list)>=16 or len(zero_ha_list)>=16 or len(zero_fpc_list)>=16:
            if item not in error_testbed_list:
                error_testbed_list.append(item)
...
def write_html(start_date,year_start,end_date,year_end,device_name_dict,name_list,cnrd_owner_dict,tcs_owner_dict):
    ...
    for key, value in device_name_dict.items():
        if key!='All' and key!='Error' and key!='CNRD' and key!='TCS':
            htmlfile.write("""<a href="http://10.208.176.205/%s_status.html"><font size="4" face="arial" color="#1f77b4">%s Testbeds (%s)</font></a>""" % (key,key,len(value)))
            htmlfile.write('&nbsp;&nbsp;')
            htmlfile.write('\n')
    htmlfile.write("""<a href="http://10.208.176.205/tcs_testbeds.html"><font size="4" face="arial" color="#1f77b4">TCS Testbeds (%s)</font></a>""" % len(device_name_dict['TCS']))  
    htmlfile.write('&nbsp;&nbsp;')
    htmlfile.write('\n')
    htmlfile.write("""<a href="http://10.208.176.205/cnrd_testbeds.html"><font size="4" face="arial" color="#1f77b4">CNRD Testbeds (%s)</font></a>""" % len(device_name_dict['CNRD']))  
    htmlfile.write("""</p>""")
    htmlfile.write("""<a href="http://10.208.176.205/All_status.html"><font size="4" face="arial" color="#1f77b4">All Testbeds (%s)</font></a>""" % len(device_name_dict['All']))  
    htmlfile.write("""</p>""")
    htmlfile.write("""<p style="text-align: center;">""")
    htmlfile.write("""<a href="http://10.208.176.205/session-ha-fpc_log.csv"><font size="4" face="arial" color="#1f77b4">Download Log File for Detailed Info</font></a>""")  
    htmlfile.write('&nbsp;&nbsp;')
    htmlfile.write('\n')
    htmlfile.write("""<a href="http://10.208.176.205/Error_status.html"><font size="4" face="arial" color="#1f77b4">Testbeds with Error State (%s)</font></a>""" % len(device_name_dict['Error']))  
    htmlfile.write("""</p>""")
    for item in name_list:
        htmlfile.write("""<a href="http://10.208.176.205/%s.html"><img src="%s_version.png" width="600" height="320" /></a>""" % (item.split('_')[0],item.split('_')[0]))
    ...
    for key, value in cnrd_owner_dict.items():
        if i==4 or i==7:
            htmlfile.write("""</p>""")
            htmlfile.write("""<p style="text-align: center;">""")
        htmlfile.write("""<a href="http://10.208.176.205/%s_status.html"><font size="4" face="arial" color="#1f77b4">%s's Testbeds (%s)</font></a>""" % (key,key,len(value)))
        htmlfile.write('&nbsp;&nbsp;')
        htmlfile.write('\n')
        i=i+1
    htmlfile.write("""</p>""")
    for key, value in cnrd_owner_dict.items():
        ...
        htmlfile.write("""<a href="http://10.208.176.205/%s_status.html"><img src="%s_version.png" width="600" height="320" /></a>""" % (key,value[0]))
    htmlfile.write("""</center></body>
    </html>""")
    htmlfile.close()
...
def write_version_csv(df,name_list):       
    ...
    DataFrame.to_csv('version_info.csv')
...
def write_session_csv(df1):
    ...
    df1.to_csv('session-ha-fpc_info_init.csv')
    i=0
    ...
    for fpc_item in df1['fpc']:
        ...
        i+=1
    df1.to_csv('session-ha-fpc_info.csv')
...
def write_log_csv(df_log1,df_log2,df_log3):
    ...
    if platform.system()=='Linux':
        result.to_csv('/var/www/html/session-ha-fpc_log.csv')
    else:
...
def bar_chart(version_list_bar,uptime_list,device_name,start_date,year_start,end_date,year_end):
    ...
    plt.tight_layout()
    if platform.system()=='Linux':
        plt.savefig("/var/www/html/%s_version.png" % device_name, dpi=200, transparent=True, pad_inches = 0)
    else:
...
def bar_line_chart(x,y1,y2,y3,version_list,device_name,version_labels,time_values,version_list_short,uptime_list,start_date,year_start,end_date,year_end,version_middle_index_list):
    ...
    #ax2=plt.subplot(211)
    if platform.system()=='Linux':
        plt.savefig("/var/www/html/%s_session.png" % device_name, dpi=200, transparent=True, pad_inches = 0)
    else:
...
def status_line_chart(x,y1,y2,y3,version_list,device_name,start_date,year_start,end_date,year_end,version_middle_index_list):
    ...
    plt.tight_layout()
    if platform.system()=='Linux':
        plt.savefig("/var/www/html/%s_status.png" % device_name, dpi=200, transparent=True, pad_inches = 0)
    else:
...
#print timeline_list
if platform.system()=='Linux':
    if os.path.exists('/home/regress/Ansible/facts/device-facts.csv')==True:
        os.remove('/home/regress/Ansible/facts/device-facts.csv')
else:
    if os.path.exists('/Users/ryanliu/Downloads/Ansible/facts/device-facts.csv')==True:
        os.remove('/Users/ryanliu/Downloads/Ansible/facts/device-facts.csv')
for f in csv_list:
    df=pd.read_csv(f)
    df.to_csv('device-facts.csv', index=False, header=False, mode='a+')
df_init=pd.read_csv('device-facts.csv', header=None, names=['name','version','booted_time','type','session','HA','fpc','time'],low_memory=False)
df=df_init.copy()
...
df_log2=pd.read_csv('session-ha-fpc_info.csv',low_memory=False)
df_log3=pd.read_csv('session-ha-fpc_info_init.csv',low_memory=False)
df_error=df_log2.copy()
write_log_csv(df_log1,df_log2,df_log3)


df_version_info=pd.read_csv('version_info.csv', low_memory=False)
