...
name_list_minipdt=['zhaoguo-2','weiguo-2','chuguo-2','hanguo-2','gw-minipdt-quincy1','pdt-xian','pdt-tongchuan','slt28-palm','pdt-xian-2','pdt-tongchuan-2','nonibert','qinguo-2','shanhai','qinguo','qiguo-2']
...
name_list=['zhaoguo-2','weiguo-2','chuguo-2','hanguo-2','gw-minipdt-quincy1','pdt-xian','pdt-tongchuan','slt28-palm','pdt-xian-2','pdt-tongchuan-2','nonibert','qinguo-2','shanhai','qinguo','qiguo-2']
...
def get_date_list(interval):
    ...
    for i in range(interval):
        ...
        #now=datetime.datetime(2019, 5, 29, 23, 59, 59, 36867)
        #today=datetime.date(now.year,now.month,now.day)  
...
def write_html(start_date,year_start,end_date,year_end,device_name_dict,name_list,cnrd_owner_dict,tcs_owner_dict):
    ...
    htmlfile=open('/var/www/html/minipdt.html', 'w')
    htmlfile.write("""<html><body><head><title>MiniPDT Testbeds Status</title>
    </head>
    <center>""")
    htmlfile.write("""<h1><font size="5" face="verdana" color="#1f77b4">MiniPDT Testbeds Monthly Status (%s/%s to %s/%s)</font></h1>""" % (start_date,year_start,end_date,year_end))
    for item in name_list:
        htmlfile.write("""<a href="http://10.208.176.205/log/%s.html"><img src="log/%s_version.png" width="600" height="320" /></a>""" % (item.split('_')[0],item.split('_')[0]))
    htmlfile.write("""</center></body>
    </html>""")
    htmlfile.close()
...
def bar_line_chart(x,y1,y2,y3,version_list,device_name,version_labels,time_values,version_list_short,uptime_list,start_date,year_start,end_date,year_end,version_middle_index_list):
    ...
    #ax.bar(x, y, color='#0066FF', label='Up Time')
    for a,b in zip(x1,y):
        ax1.text(b+0.08, a, '%.1f' % b, ha='left', va= 'center',fontsize=8)
    ...
    for i in range(len(version_list)):
        if version_middle_index_list[i]!='':
            version_list[i]=version_middle_index_list[i]
    for i in range(len(version_list)):
        if i!=len(version_list)-1:
            if version_list[i]=='-----------------':
                version_list[i]=''
                version_list[i+1]=''
        #if '_' in version_list[i]:
        #    version_list[i]=version_list[i].split('_')[0]
        #if '-' in version_list[i]:
        #    version_list[i]=version_list[i].split('-')[0]
        if '/' not in version_list[i]:
            version_list[i]='                    %s' % version_list[i]
        i=i+1
    plt.xticks(x,version_list,rotation=270,fontsize=12)
    ...
    #plt.xticks(x,version_list,rotation=270,fontsize=12)
    for i in range(len(version_middle_index_list)):
        if '_' in version_middle_index_list[i]:
            version_middle_index_list[i]=version_middle_index_list[i].split('_')[0]
        if '-' in version_middle_index_list[i]:
            version_middle_index_list[i]=version_middle_index_list[i].split('-')[0]
        i=i+1
    ##plt.xticks(x,version_middle_index_list,rotation=0,fontsize=12)
    #plt.xlabel('Blue Line (Total Sessions Number), Green Line (HA Status:Pass=1,Fail=0), Red Line (PIC Status:Pass=2,Fail=0)',fontsize=18)
